...
from formulas.combinatorics import (
    combinations_without_rep as C,
    permutation_with_rep as _P,
)
from typing import List, Optional


def bernoulli_formula(
    n: int, p: float, start: int, end: Optional[int] = None
) -> float:
    if end is not None:
        result = 0
...
def moivre_laplace_integral_formula(
    m1: int, m2: int, p: float, n: int
) -> float:
    q = 1 - p
